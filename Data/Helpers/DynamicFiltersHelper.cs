...
public static class DynamicFiltersHelper
{
    ...
    /// </summary>
    /// <param name="student">The student with all of its profile settings and Roles.Role properties</param>
    ...
    {
        if (student.UserType.HasFlag(UserType.Company) || (student.Roles.Count > 0 && student.Roles.Any(e => e.Role.Name == "Company")))
            throw new InvalidEnumArgumentException("Student cannot be a company");

        excludeKeys ??= new[] { "EducationLevel", "Description" };

        Expression<Func<User, bool>> filter = e => e.UserType == UserType.Company && !e.Flirts.Any(o => o.CompanyId == o.Id && o.Status > FlirtStatus.Finished);
